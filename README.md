...
│   ├── video_inference.py       # 视频全帧离线推理管线（YOLO / SAM3，含跟踪+深度模式）
│   ├── sam3/models/model.pt     # SAM3 权重（约 3.3GB，不入库，需自行放置）
│   ├── yolo11/                  # YOLO 业务模型安装 / 权重目录（运行时生成）
│   └── yolo_depth/              # 通用深度跟踪抽象层：检测器 / 跟踪器 / 深度估计器 / 测距测速
...
3. 选择**任务模式**：
   - `普通检测`：仅逐帧检测并绘框
   - `跟踪+深度`：为检测目标赋予稳定 ID，并叠加深度距离与速度
4. 设置置信度，点击 `开始全帧推理`
5. 实时进度（SSE）+ ETA，处理完成后产出带标注的 AI 视频
6. 原视频与 AI 视频**左右并排**，可勾选「同步联动」一起播放/拖动对比
...
「跟踪+深度」是一个平台级通用能力，与所选检测器正交：

- **检测**：复用当前选择的检测器 —— YOLO（任意 Ultralytics YOLO 模型）或 SAM3（任意文本类别）
- **跟踪**：ByteTrack 为同一目标赋予稳定跟踪 ID
- **深度**：用通用单目深度模型 **Depth Anything v2** 估计深度图，对检测框取中值作为目标距离（m）
- **速度**：用 Δd/Δt 估算接近/远离速度（km/h，径向演示级估计）
- 逐目标标注 `ID8 car 4.5m 18km/h 靠近`，与原文左右/四屏对比

布局为「引擎 = 检测器、任务模式 = 检测/跟踪+深度」两层：
引擎只管「检出什么」，任务模式决定「检出后要不要跟踪+测距」。因此用任意 YOLO 业务模型或 SAM3 文本类别（如 person、bicycle）都能做跟踪+深度，不受车辆类别限制。

> 深度模型无需单独放置权重：首次使用 Depth Anything 时从 HuggingFace 自动下载。单目测距与速度是演示级估计，非精密计量（供交通演示 / 教学用）。
