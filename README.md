...
引擎下拉选择 **「车辆深度跟踪」** 后，以离线任务方式整段跑完，自动检测 car/bus/truck/moto：

- 用 ByteTrack 赋予每个车辆稳定 **跟踪 ID**
- 用 YOLO26 单目深度模型估相机到车距（**距离 m**）
- 用 Δd/Δt 估接近/远离速度（**km/h**，径向演示级估计）
- 逐车中文标注 `ID8 car 4.5m 18km/h 靠近`，与原文左右/四屏对比

引擎自包含两个权重（`plugins/yolo_depth/weights/`）：检测 `yolo26n.pt` + 深度 `yolo26n-depth.pt`。
**这两个权重随仓库提交**，作为平台「`*.pt` 不入库」约定的显式例外（见 `.gitignore`）。缺失时会给出清晰错误提示。

> 单目测距与速度是演示级估计，非精密计量（供交通演示 / 教学用），更贴近源项目 `yolo-vehicle-depth-track` 语义。
